...
$.fn.wsAnchor = function(Attrs){
    ...
    function __setActive(Screen) {
        try {
            ...
            console.log('JQUERY.WS: Возможно отсутствуют контейнеры или не указан id, так же, проверьте правильность ввода класса контейнера, и перезагрузите страницу.');
            return;
        }
...
jQuery(document).ready (function () {
    ...
    // Краткое описание
    (function () {
        var main = '.wsj-tab__btn';
        var q_main = $(main);

        if (q_main.length == 0) return;

        var active_css = 'wsj-tab__btn-active';

        q_main.click(function() {

            var interval = setInterval(function() {
                if (q_main.hasClass(active_css)) {
    ...
    // ---------------------------------------
    $( '.wsb-assortment__item_certificate a' ).click( function( e ) {
    	e.preventDefault();
    	$.swipebox( [
    		{ href:'images/sertifikat.jpg', title:'Сертификат' },
    	] );
    } );
    // ---------------------------------------
    ...
    // ---------------------------------------
    // Добавление белой тени к элементам
    (function () {

        var main    = '.wsj-general-item';
        var q_main  = $(main);

        if (q_main.length == 0) return;

        var factor  = 1.47;
        var parent  = '.wsb-general__item';

        q_main.each(function() {
            var self   = $(this);
            ...
            self.css({
                width:      width  * factor,
                height:     (height * factor) * 1.3,
                marginTop:  -1 *(((height * factor) * 1.3) / 2),
                marginLeft: -1 *((width  * factor) / 2)
            });
        })
    })();
    // ---------------------------------------
    // Слайдер
    (function () {
        var main = '.wsj-assortment__slider';
        var q_main = $(main);

        if (q_main.length == 0) return;

        var item_class   = '.wsb-assortment__item';
        var center_class = '.wsj-active-tab';
        var center_css   = 'wsj-active-tab';
        var arr_block    = false;
        var swipe        = false;
        ...
        q_main.each(function() {
            ...
            $(self).attr('slides-count', $(self).find(item_class).length - 1);

            $(self).on('init', function(){
                //if (WidthWindow >= 768) {
                    var next_arr = $('<button class="slick-arrow slick-next" />');
                    var prev_arr = $('<button class="slick-arrow slick-prev" />');

                    $(self).append(next_arr);
                    $(self).prepend(prev_arr);

                    next_arr = $(self).find('.slick-next');
                    prev_arr = $(self).find('.slick-prev');

                    next_arr.click(function() {
                        if (arr_block) return;

                        clearCenter.call(self, function() {
                            arr_block = true;
                            nextSlide.call(self, function() {
                                setCenter.call(self);
                            });
                        });
                    });
                    ...
                    $('.ws-tab-wrapper').on('wsOnTabChange', function() {
                        next_arr.click();
                    });
                //}
                ...
                if (WidthWindow < 1025) {
                    ...
                    $(self).on('beforeChange', function(event, slick, currentSlide, nextSlide){
                        $(self).find(center_class).removeClass(center_css);
                    });
                    $(self).on('afterChange', function(event, slick, currentSlide, nextSlide){
                        $(self).find('.slick-center').addClass(center_css);

                        $('.ws-tab-wrapper')[0].wsSetWrapperHeight();

                        if ($(self).find('.slick-list').height() > 1200) {
        ...
        function prevSlide(func) {
            var slides_count  = $(this).attr('slides-count');
            var current_slide = $(this).slick('slickCurrentSlide');
            var fix_class     = 'ws-slide-fix';
            var track         = $(this).find('.slick-track');

            if (current_slide == 0) {
                track.addClass(fix_class);
            } else {
                track.removeClass(fix_class);
            }
        ...
        function nextSlide(func) {
            ...
            if (slides_count - 1 == current_slide) {
                track.addClass(fix_class);
            } else {
                track.removeClass(fix_class);
            }
        ...
        function clearCenter(func) {
            var center = $(this).find('.slick-center');
            center.removeClass(center_css);
    ...
    // ---------------------------------------
    // Появление маркеров на карте
    (function () {
        var main = '.wsj-member-container';
        var q_main = $(main);

        if (q_main.length == 0) return;

        var item = '.wsb-member__item';
        var height = $('.wsb-member').height();
        var q_items = q_main.find(item);
        ...
        $(document).scroll(function() {
            ...
            if (enable() && !animation_over)
                showMarkers();
        });
        // ------------------
        function showMarkers() {
            q_items.each(function() {
                var self = $(this);
                self.delay(getRand(100, 2000)).fadeIn(300);
            });
            animation_over = true;
        }
        // ------------------
        function enable() {
            ...
            if ($(document).scrollTop() > q_main.offset().top -  height * 1 )
                return true;
    ...
    // Параллакс в хедере
    (function () {
        var main = '.wsj-oil__wrap';
        var q_main = $(main);

        if (q_main.length == 0) return;

        var mouseport = $('.wsb-general');
        $(main).parallax(
            { mouseport: mouseport},
            { xparallax: '100px',    yparallax: '5px' }
        );
    })();
    (function () {
        var main = '.wsj-plx-bg img';
        var q_main = $(main);
    ...
    $(function(){
		...
		//$('.wsg-captcha').text(result);
		$('.wsg-input-captcha_h').val(result);
		$('.wsg-input-captcha_h_d').val(result);
    });
    // ---------------------------------------
    // Ajax Отправка форм
    (function () {
        ...
        var popup_wrapper = '.wsj-popup-wrapper';
        var popup_thank   = '#ws-thank';
        var t_ajax_send   = 'ws-ajax-send';

        q_main.on('submit', function() {
            var self = $(this);

            if (checkForm.call(self))
                send.call(self);
        ...
        function checkForm() {
            ...
            var form_error_css = 'ws-form-error';

            name.each(function() {
                $(this).removeClass(form_error_css);

                if ($(this).val().length == 0) {
                    state = false;
                    $(this).addClass(form_error_css);
                };
            });
        ...
        // ------------------
        function closePopup() {
            var popup = this.closest(popup_wrapper);
        ...
        // ------------------
        function showThank() {
            $(popup_thank)[0].wsOpen();
        }
        // ------------------
        function send() {
            var self = this;
			var form_id=$(this).find('input[name="form-action"]').val();
            ...
            $.ajax({
                ...
                success: function() {
                    ...
                    showThank();
                    $(self).trigger(t_ajax_send);
					if(form_id=="contact")
					{
						ga('send','event','forms','call_back');
						yaCounter44194494.reachGoal('call_back');
					}
					if(form_id=="testpart")
					{
						ga('send','event','forms','test_part');
						yaCounter44194494.reachGoal('test_part');
					}
					if(form_id=="price")
					{
						ga('send','event','forms','price');
						yaCounter44194494.reachGoal('price');
					}
					if(form_id=="cooperator")
					{
						ga('send','event','forms','cooperator');
						yaCounter44194494.reachGoal('cooperator');
					}
					if(form_id=="privilegies")
					{
						ga('send','event','forms','privilegies');
						yaCounter44194494.reachGoal('privilegies');
					}
					if(form_id=="work")
					{
						ga('send','event','forms','work');
						yaCounter44194494.reachGoal('work');
					}
					if(form_id=="brand")
					{
						ga('send','event','forms','brand');
						yaCounter44194494.reachGoal('brand');
					}
                }
            });
        }
    ...
    (function () {
        ...
        $(q_main).styler({
            ...
            onSelectClosed: function() {
                var text = $(this).find('.sel').data('text');
                var target = $('.wsj-client-type__target');

                target.text('');
                target.text(text);

                $('.wsj-open-pdf').data('type', $(this).find('.sel').data('type'));
            }
        });
    ...
    // ---------------------------------------
    (function () {
        var main = '#ws-hidden-submit_target';
        var q_main = $(main);

        if (q_main.length == 0) return;

        var form = q_main.closest('form');
        var submit_btn = $('.ws-hidden-submit');

        q_main.click(function() {
            var type    = $('.selected.sel').data('type');
            $(this).attr('href', 'upload/'+type+'.pdf');

            if (!checkForm.call(form))
                return false;
            else {
                submit_btn.click();
            }
